...
//        orderService.saveOrder(order);
    ...
    public String changeOrderPost(@PathVariable(value = "numberOrder") int numberOrder, @ModelAttribute("order") Order orderNew, BindingResult bindingResult, Model model) {
//        Order order = orderService.findOneOrder(numberOrder);
        orderNew.setId(orderService.findOneOrder(numberOrder).getId());
    ...
    public String addNewElementForOrderPost(@RequestParam(required = true) int numberOrder,
        ...
        if (typeMaterial.equals("Выбирите материал")) {
            allModel(model);
            model.addAttribute("order", orderService.findOneOrder(numberOrder));
            model.addAttribute("materialError", "Не выбран материал!");
            return "addNewElementForOrder";
        }
        ...
        detailInfo.setMaterial(materialService.findByName(typeMaterial));
        detailInfo.setCount(Integer.parseInt(countDetail));
        detailInfo.setDateStart(LocalDateTime.parse(dateStart));
    ...
    public String deleteElementFromOrderPost(@PathVariable(value = "numberOrder") int numberOrder, @PathVariable(value = "increment") int increment) {
        Order order = orderService.findOneOrder(numberOrder);
        List<DetailInfo> detailInfos = order.getDetailInfos();
